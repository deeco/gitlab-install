...
Vagrant.configure("2") do |config|
    ...
    config.vm.define "gitlab" do |web|
        ...
        config.vm.provision "file", source: "./gitlab/nginx-config/artifactory.key", destination: "~/artifactory.key"

        # copy certs for artifactory
        config.vm.provision "file", source: "./gitlab/nginx-config/artifactory.crt", destination: "~/artifactory.crt"
